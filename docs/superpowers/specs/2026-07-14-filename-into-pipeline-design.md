...
`2022年-精骐&捷美-产品价格表.csv`：表格每一行都是「精骐&捷美」的产品，但**归属（品牌/公司）只写在文件名上，行内容里完全没有**。当前管线下每行 chunk = `表头 + 该行`，既不含品牌、上下文化也看不到文件名 → RAG 检索/问答时模型不知道这些产品是谁的，用户问「精骐&捷美的某产品多少钱」根本命中不了。这类「归属只在文件名、正文缺失」是本次改动要解决的核心场景，不只是「让模型知道来源」，而是要让上下文化**主动把文件名里的归属补进每个 chunk**。
...
- **归属补全（核心）**：上下文化 prompt 明确指示——**当片段本身没写明品牌/公司/时间等归属，而文档标题/文件名里有，就把这些归属补进上下文描述**（如价格表某行未写品牌 → 从文件名补「精骐&捷美」）。这是本次的主诉求，优先级高于"简洁"。
- **去重策略（收敛版）**：不再用「完全不许提文件名」那种粗指令（会连带压掉品牌补全）。改为「**不必逐字照搬整个文件名**（前缀已给出），但要善用其中的品牌/公司/时间补全归属」。因此可能出现「机械前缀里的原文件名」与「LLM 描述里重述的品牌」并存——这对检索是增益（品牌 token 加权），可接受，不做去重。
...
- **抽出纯函数** `buildContextualizeContent(fullDoc, chunk, title?)`（**导出**，与 `buildAnswerSystemPrompt` 同款可测），返回 `messages[0].content` 的两块文本数组：
  ...
  - 第 2 块 = 上下文说明指令，把「归属补全」写进要点列表：
    > 请阅读上述完整文档，为下面片段生成上下文说明（来源定位 + **归属补全** + 核心对象/时间 + 指代消解）：
    > **归属补全**：若片段本身没写明所属品牌/公司/时间等，而文档标题/文件名里有，就在描述中补上（例：价格表某行没写品牌，则从文件名补出该品牌）。
    > 其后接现有的 `<chunk>…</chunk>`。
- `contextualize()` 改为调用 `buildContextualizeContent(...)` 填 `messages`，逻辑等价。
- system 提示调整：保留「只输出描述本身、50~100 字、单段、无『该片段…』前缀」，末尾追加一句归属+去重收敛指令：
  > 若片段缺失品牌/公司/时间等归属而文档标题里有，请补进描述；不必逐字照搬整个文件名。
...
**表格行（动机例子）覆盖情况**：
- **小表（行 ≤ `maxLlmRows`，默认 400）**：每行走 LLM 上下文化，①让模型看到文件名 + 归属补全指令 → 每行前缀会带上从文件名推断的品牌（「精骐&捷美2022年产品价格表中，产品A…」）。这是最佳形态。
- **大表（行 > 400）**：为控成本行级回退确定性前缀 `《文件名》· sheet/章节`（无 LLM）。品牌不再由模型语义重述，但**文件名整串（含"精骐&捷美"）仍在每行 chunk 的前缀里** → 向量 + BM25 命中不受影响，Opus 拿到行时也能从前缀里的文件名读出归属。即大表退化为"raw 文件名归属"，仍可用；`maxLlmRows` 可按需调高换取全行 LLM 归属。
...
- **`buildContextualizeContent`（`llm-client.test.ts` 扩充）**：
  ...
  - 第 2 块含 `<chunk>` 与 `chunk` 内容，且含「归属补全」要点（断言关键词，如 `归属`/`品牌`）。
